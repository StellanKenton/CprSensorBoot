...
1. 收到 `0xEA` 后返回当前支持的最大单包长度。
2. 收到 `0xEB` 后校验文件大小是否小于 `APP_FLASH_APP1_DATA_MAX_SIZE`，再写 `app2` 头部的固定字段。
3. 收到 `0xEC` 后根据 `stUpdateImageHeader.writeOffset` 返回可续传位置。
4. 收到 `0xED` 后先做分包 CRC16 校验，再把 payload 写入 `app2` 数据区。
5. 全部收完后做整镜像 CRC32 校验；成功则把 `imageState` 改成 `READY`。
6. 最后写 `stUpdateBootRecord.requestFlag = E_UPDATE_BOOT_FLAG_APP_REQUEST`，然后触发软件复位。
