...
#define WIFI_TCP_SERVER_DEFAULT_PORT "5000"
...
static void Wireless_GetTcpServerPort(uint8_t *portBuffer, uint16_t bufferSize);
static uint8_t Wireless_OpenTcpServer(void);
/*!
* \brief 串口接收DMA中断处理
* \param   none
* \return none
...
uint8_t Wifi_Init()
{
    /* Keep BLE alive while refreshing only the STA-side state. */
    (void)WireLess_Modlue_AT_Check(&Ring_WireLessComm,"AT+QICLOSE=0\r\n",400,wRecv_Char);
    (void)WireLess_Modlue_AT_Check(&Ring_WireLessComm,"AT+QSTASTOP\r\n",400,wRecv_Char);
    osDelay(100);
...
static void Wireless_GetTcpServerPort(uint8_t *portBuffer, uint16_t bufferSize)
{
    if ((portBuffer == NULL) || (bufferSize == 0U)) {
        return;
    }

    memset(portBuffer, 0x00, bufferSize);
    if (Wifi_Module.Port[0] != '\0') {
        strncpy((char *)portBuffer, (const char *)Wifi_Module.Port, bufferSize - 1U);
    }
    else {
        strncpy((char *)portBuffer, WIFI_TCP_SERVER_DEFAULT_PORT, bufferSize - 1U);
    }
}

static uint8_t Wireless_OpenTcpServer(void)
{
    static const char *gTcpServerCmdFormats[] = {
        "AT+QIOPEN=0,\"TCPSERVER\",%s\r\n",
        "AT+QIOPEN=0,\"TCP SERVER\",%s\r\n",
    };
    uint8_t lPort[8];
    uint32_t lIndex;

    Wireless_GetTcpServerPort(lPort, (uint16_t)sizeof(lPort));

    for (lIndex = 0U; lIndex < (uint32_t)(sizeof(gTcpServerCmdFormats) / sizeof(gTcpServerCmdFormats[0])); lIndex++) {
        memset(su8_AT_Command, 0x00, sizeof(su8_AT_Command));
        snprintf((char *)su8_AT_Command,
                 sizeof(su8_AT_Command),
                 gTcpServerCmdFormats[lIndex],
                 lPort);
        if (WireLess_Modlue_AT_Check(&Ring_WireLessComm, (char *)su8_AT_Command, 1000, wRecv_Char) == wRecv_Char) {
            if ((strcmp((const char *)Ring_WireLessComm.RevData, "OK") == 0) ||
                (strcmp((const char *)Ring_WireLessComm.RevData, "CONNECT") == 0)) {
                return 1;
            }
        }
    }
    ...
    switch(s_Wifi_State) {
        ...
        case WIFI_CONNECT_AP_STATE:
            ...
            if(s_Err_Cnt >= 5) {
                s_Err_Cnt = 0;
                s_Wifi_State = WIFI_INIT_STATE;
                SEGGER_RTT_printf(0,"Connect AP Failed, retry WiFi only\r\n");
            }
            break;
        case WIFI_GET_IP_STATE:
            ...
            if(s_Err_Cnt >= 5) {
                s_Err_Cnt = 0;
                s_Wifi_State = WIFI_INIT_STATE;
                SEGGER_RTT_printf(0,"Get IP Failed, retry WiFi only\r\n");
            }
            break;
        case WIFI_CONNECT_TCP_STATE:
            if(Wireless_OpenTcpServer() == 1) {
                s_Wifi_State = WIFI_CONNECTED_STATE;
                SEGGER_RTT_printf(0,"TCP Server Ready\r\n");
                s_Err_Cnt = 0;
                break;
            }
            osDelay(1000);
            ...
            if(s_Err_Cnt >= 5) {
                s_Err_Cnt = 0;
                s_Wifi_State = WIFI_INIT_STATE;
                SEGGER_RTT_printf(0,"TCP Server Open Failed, retry WiFi only\r\n");
            }
            break;
        ...
        case WIFI_INIT_MQTT_STATE:
            ...
            if(s_Err_Cnt >= 5) {
                s_Err_Cnt = 0;
                s_Wifi_State = WIFI_INIT_STATE;
                SEGGER_RTT_printf(0,"MQTT Init Failed, retry WiFi only\r\n");
            }
            break; 
        case WIFI_INIT_HTTP_STATE:
            ...
            if(s_Err_Cnt >= 5) {
                s_Err_Cnt = 0;
                s_Wifi_State = WIFI_INIT_STATE;
                SEGGER_RTT_printf(0,"HTTP Init Failed, retry WiFi only\r\n");
            }
            break;
